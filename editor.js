...
function readImage() {
      if (this.files && this.files[0]) {
            ...
            FR.onload = (e) => {
                  ...
                  turnOnUI(); // turn on user interface
            };
...
function turnOnUI() {
      ...
      function changePixel(i, minBorder, maxBorder, value, sign = 1) {
            for (let j = 0; j < 3; j++) /*change r,g and b of a given pixel*/ {
      ...
      function changeContrast() {
            ...
            for (let i = 0; i < imageData.data.length; i += 4) {
                  ...
                  sign = (averagePixelRgb > 127) ? 1 : -1; // increase contrast if a pixel is bright or subtrack if it's dark

                  if (contrast >= 0) /*check if you want to increase contrast*/ {
                        changePixel(i, 0, 255, contrast, sign); //increase contrast
      ...
      function changeBlur() {
            ...
            for (let i = 0; i < newImageData.data.length; i++) {
                  newImageData.data[i] = parseInt((
                        /*middle px*/newImageData.data[i] +
                        /*left px*/newImageData.data[i - 4] +
                        /*right px*/newImageData.data[i + 4] +
                        /*up left px*/newImageData.data[i - imgWidth - 4] +
                        /*up px*/newImageData.data[i - imgWidth] +
                        /*up right px*/newImageData.data[i - imgWidth + 4] +
                        /*down left px*/newImageData.data[i + imgWidth - 4] +
                        /*down px*/newImageData.data[i + imgWidth] +
                        /*down right px*/newImageData.data[i + imgWidth + 4]) / 9);
            }
            ctx.putImageData(newImageData, 0, 0);
      }
